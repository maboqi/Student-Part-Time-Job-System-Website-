...
try {
    ...
    $countSql = "SELECT COUNT(*) FROM application WHERE status IN ('Accepted', 'Validated', 'AdminDenied')";
    if (!empty($searchTerm)) {
    ...
    //用不上了$countStmt->bindParam(':employerid', $user_id, PDO::PARAM_INT);
    ...
    $sql = "SELECT * FROM application WHERE status IN ('Accepted', 'Validated', 'AdminDenied')";
    if (!empty($searchTerm)) {
        ...
        <div class="header">
            ...
            <form method="GET" class="search-bar">
                ...
                <select name="status">
                    ...
                    <option value="Denied" <?php echo ($statusFilter == "AdminDenied") ? "selected" : ""; ?>>AdminDenied</option>
                </select>
                ...
                foreach ($applications as $application) {
                    ...
                    <div class="list-item" data-toggle="modal" data-target="#myModal" onclick="updateModalContent('<?php echo htmlspecialchars(json_encode($application)); ?>')">
                        ...
                        <div class="item-content">
                            <div class="item-main-content">Apply ID: <?php echo htmlspecialchars($application['applyid']); ?> | Job: <?php echo htmlspecialchars($application['jobtitle']); ?></div>
                            <div class="item-sub-content">
                                ...
                                <div class="sub-item">Applicant Name: <?php echo ($application['student']); ?></div>
                                <div class="sub-item">Job_Title: <?php echo ($application['jobtitle']); ?></div>
                                <div class="sub-item">Status: <?php echo ($application['status']); ?></div>
                            </div>
                        </div>
                        <button type="submit" class="sign-up" name="create" onclick="window.location.href='validation.php?id=<?php echo $application['applyid']; ?>'">Detail</button>
                    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="buttons">
                    <form method="POST" action="update_admin_status.php">
                        <input type="hidden" id="in-applyid" name="applyid">
                        <input type="hidden" id="in-status" name="status">
                        <button class="btn btn-primary update-btn" type="submit" onclick="setStatus('Validated')">Validate</button>
                        <button class="btn btn-danger update-btn" type="submit" onclick="setStatus('AdminDenied')">Deny</button>
                    </form>
