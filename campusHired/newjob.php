...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $lv = $_POST['lv'];
        ...
        // 插入数据，jobid 会自动生成
    $stmt = $conn->prepare("INSERT INTO job (shop, employer, employerid, salary, location, des, starttime, endtime, jobtype, jobtitle, lv) VALUES (:shop, :employer, :employerid, :salary, :location, :des, :starttime, :endtime, :jobtype, :jobtitle, :lv)");
    $stmt->bindParam(':employerid', $id, PDO::PARAM_INT);
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        // 插入数据到数据库
        $stmt = $conn->prepare("UPDATE job SET shop = :shop, pic_data = :pic_data, mime_type = :mime_type WHERE jobid = :jobid;");
        ...
        echo "Image upload error: code " . $file['error'];
    }


    $workingDays = isset($_POST['working-days']) ? $_POST['working-days'] : '';

        // 插入数据到数据库（假设你已经有一个 job 表）
    $stmt = $conn->prepare("UPDATE job SET working_days  = :working_days WHERE jobid = :jobid");
    $stmt->bindParam(':working_days', $workingDays, PDO::PARAM_STR);
    $stmt->bindParam(':jobid', $jobid, PDO::PARAM_INT);

    if ($stmt->execute()) {
        echo "Workday saved successfully!";
    } else {
        echo "Save failed.";
    }
...
<div class="main-content">
    ...
    <div class="purple-background">
        <div class="form-container">
            ...
            <form action="newjob.php" method="POST" class="form" enctype="multipart/form-data">
                ...
                <div id="tab_general" class="tab-content active">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="shop-name" name="shop">
                    </div>
                    <div class="form-group">
                        <label for="employer-name">Employer Name</label>
                        <input type="text" id="employer-name" name="employer" value="<?php echo $result["lastname"]; ?>" readonly>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="tel" id="phone" name="phone" value="<?php echo $result["phone"]; ?>" readonly>
                    </div>
                    <div class="form-group">
                        <label for="address-line">Address</label>
                        <input type="text" id="address-line1" name="location">
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="file" id="logo-upload" name="image" accept="image/*" onchange="previewLogo(event)" required>
                ...
                <div class="form-group">
                    ...
                    <div id="logo-preview-container" style="margin-top: 10px;">
                     <img id="logo-preview" src="" alt="Logo Preview" style="max-width: 50px; max-height: 50px; display: none; border: 1px solid #ddd; padding: 5px; border-radius: 5px;">
                 </div>
             </div>
         ...
         <div id="tab_des" class="tab-content">
            <div class="form-group">
                ...
                <textarea id="description" name="des" class="large-textarea"></textarea>
            </div>
