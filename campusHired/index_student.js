...
.then(data => {
    ...
    currentUser.studentid = data.user_id;
    currentUser.student = data.user_name;
    ...
    get(chatroomsRef).then(snapshot => {
        ...
        data.users.forEach(user => {
                    ...
                    // 生成聊天室ID
            const chatroomId2 = `chat_S${currentUser.studentid}_E${user.employerid}`;
            const chatroomId = `chat_E${user.employerid}_S${currentUser.studentid}`;
            ...
            if (!(existingChatrooms[chatroomId] || existingChatrooms[chatroomId2])) {
                ...
                set(chatroomRef, {
                    ...
                    console.log(`Room of E${user.employer} and S${currentUser.student}`);
                }).catch(error => console.error("Failed creating room:", error));
            } else {
                console.log(`Room already existed：${currentUser.student} and ${user.employer}`);
            }
        ...
        data.users.forEach(user => {
            ...
            contact.innerText = user.employer;
...
function openChatroom(user) {
    ...
    currentChatroomId = `chat_E${user.employerid}_S${currentUser.studentid}`;
    ...
    // 更新聊天窗口标题
    document.querySelector("#chat-room h2").innerText = `Chat with ${user.employer}(Employer)`;

    fetch("fetch_apps.php")
    .then(response => response.json())
    .then(data => {
        console.log('jobinfos:', data.jobinfo);
        //console.log('employerinfos:', data.employerinfo);
        //console.log('his:', data.hi);
        data.jobinfo.forEach(job => {


            document.getElementById("picture").src = job.pic_data;
            ...
            //document.getElementById("phone").textContent = data.hi;
            //document.getElementById("email").textContent = employer.email;
            
        
    });

    fetch("fetch_apps2.php")
    .then(response => response.json())
    .then(data => {
        //console.log('jobinfos:', data.jobinfo);
        console.log('employerinfos:', data.employerinfo);
        
        data.employerinfo.forEach(employer => {


            document.getElementById("phone").textContent = employer.phone;
            document.getElementById("email").textContent = employer.email;
